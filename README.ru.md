...
```bash
task test:load                     # Запустить нагрузочное тестирование через k6 (по умолчанию: scripts/load-test.js)
task test:load SCRIPT=<путь>       # Запустить свой k6-скрипт (например, SCRIPT=scripts/stress-test.js)
```
